$(document).ready(function(){$(function(){var $commands=$('#commands-list');$.getJSON('https://raw.githubusercontent.com/MinoriBot/Shards/main/commands.json',function(data,status){$.each(data,function(_,command){$commands.append(`
            <div class="command ${command.group}" name="${command.name}">
             ...
             <div class="command-details ${command.name}-1">
              <p>Aliases: <code> ${command.aliases?.join('</code> <code>')||'None'} </code> <br><br>
                ${String(command.description).replace(/<:\w{1,}:\d{17,19}>/g,'').replace(/\[([^\]]+)\]\(([^\)]+)\)/,'<a href="$2">$1</a>').replace(/\`(\w{1,})\`/g,'<code>$1</code>')||'No description'
}<br>
                ${command.permissions?.map(x=>{return `<span class="code-style-inline-red" style="background-color:rgb(0, 102, 255)">${x.split('_').map(p=>p.charAt(0).toUpperCase()+p.slice(1).toLowerCase()).join(' ')}</span>`}).join(' ')||(command.adminOnly||command.ownerOnly?'':' <span class="code-style-inline-red" style="background-color:green">No permissions</span> ')}
                ...
                <br><br>
                Example/s:<br>
                ${command.examples.map(x=>'<code> mi!'+x+'</code>').join(' ')}
              </p>
             </div>
            </div>
          `)});$('.commands-still-loading').hide();$('.command').hide();$('.action').show();$('.command-details').hide();$('.clock-icon').hide()}).catch(()=>{$('.commands-still-loading').hide();$('.commands-loading-failed').show();});});});